...
# Flask sunucusunu başlatan bir fonksiyon
def run_flask():
    ...
    app.run(debug=True, use_reloader=False)  # use_reloader=False to avoid threading issues

# Arama fonksiyonu
def perform_search():
    search_input = entry.get()
    if search_input:
        # Elasticsearch arama isteği gönder
        ...
        # Word2Vec eş anlamlı arama isteği gönder
...
# Placeholder işlevselliği
def on_entry_click(event):
    if entry.get() == "Enter the word you want to search...":
        entry.delete(0, tk.END)  # Sil
        entry.config(fg="#d1d1d1")  # Yazı rengini aç

def on_focusout(event):
    if entry.get() == "":
        entry.insert(0, "Enter the word you want to search...")  # Yeniden ekle
        entry.config(fg="#a6a6a6")  # Yazı rengini koyu
...
entry.insert(0, "Enter the word you want to search...")  # İlk metin
entry.bind("<FocusIn>", on_entry_click)  # Tıklandığında sil
entry.bind("<FocusOut>", on_focusout)  # Tıklandığında geri ekle
